...
# Load the data
data = pd.read_csv('FBSD_GW_by_Use.csv')
...
agricultural_use = data['Agricultural']
other_use = data['Other']
...
ax.bar(years, agricultural_use, bottom=public_use + industrial_use, label='agricultural Use', color='purple')
ax.bar(years, other_use, bottom=public_use + industrial_use + agricultural_use, label='other Use', color='green')

# Set the title and labels
ax.set_title('Groundwater Use in Fort Bend County (1990 - 2023)', fontsize=12)
...
use_1990 = data.loc[data['Year'] == 1990, ['Public', 'Industrial', 'Agricultural', 'Other']].values[0]
labels = ['Public', 'Industrial', 'Agricultural', 'Other']
ax1.pie(use_1990, labels=labels, autopct='%1.1f%%', colors=['steelblue', 'orange', 'purple', 'green'])
ax1.set_title(f'1990 GW Use: {total_1990:.1f} MGD', fontsize=11)  # Include total use in title

# For 2011
ax1 = fig.add_axes([0.51, 0.62, 0.2, 0.3])  # Adjust position of pie chart
use_2011 = data.loc[data['Year'] == 2011, ['Public', 'Industrial', 'Agricultural', 'Other']].values[0]
labels = ['Public', 'Industrial', ' ', 'Other']
ax1.pie(use_2011, labels=labels, autopct='%1.1f%%', colors=['steelblue', 'orange', 'purple', 'green'])
...
use_2023 = data.loc[data['Year'] == 2023, ['Public', 'Industrial', 'Agricultural', 'Other']].values[0]
labels = ['Public', 'Industrial', 'Agricultural', 'Other']
ax2.pie(use_2023, labels=labels, autopct='%1.1f%%', colors=['steelblue', 'orange', 'purple', 'green'])
ax2.set_title(f'2023 GW Use: {total_2023:.1f} MGD', fontsize=11)  # Include total use in title

# Save the plot as a PNG file
plt.savefig('FBSD_GW_by_use_Histogram.png', dpi=300)

# Save the plot as a high-resolution PDF file
plt.savefig('FBSD_GW_by_use_Histogram.pdf', format='pdf', dpi=350)
